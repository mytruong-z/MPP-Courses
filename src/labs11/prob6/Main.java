...
public class Main {
	...
	public static <T> boolean contains2(List<T> list, T item, BiPredicate<T, T> matcher) {
		for (T listItem : list) {
			if (matcher.test(listItem, item)) {
				return true;
			}
		}
		return false;
	...
	public static void test3() {
		...
		//test using contains2
		boolean foundIt = Main.contains2(list, new Manager(1002, "Harry", 70000, 400), (e1, e2) -> e1.getId() == e2.getId());
	...
	public static void test4() {
		...
		//test using contains2
		boolean foundIt = Main.contains2(list, new CheckingAccount(1002, 35.00), (e1, e2) -> e1.getAcctId() == e2.getAcctId());
		System.out.println(foundIt);
	}
